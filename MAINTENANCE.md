...
用户点名需求「兼容手机端」。引擎本就宽度自适应（一墙脸网格、crowd 窄屏方案、滑块已支持触屏），补齐的是硬伤：

- **四页共用**：≤719px 媒体查询——标题 13px 折行（max-width 54vw）、导航收紧到右缘 16px、按钮条可折行、`overscroll-behavior: none` 防下拉刷新、`touch-action: manipulation` 去双击缩放延迟、加 `theme-color` 与纸色一致。
- **photo.js**：人物 k 值按「每行 5 人间隔」收紧（`min(行高, innerWidth/5.6)`），前排不再叠成一串；二选一卡片随宽度收缩（320px 屏也放得下，卡面内容同比缩放）；拥有栏手机端小卡上移避开按钮条；总分/结果横幅在手机上避开过滤器。
- 验证：浏览器 375×667 视口——photo 页完整玩了一轮（选 5 人→拍照→二选一→自动收卡→补位），index 截图目视核对，heads/crowd 元素坐标数值核对（标题/导航间隙 ≥64px，无截断无重叠）。
- 遗留小项：竖屏极矮（如 landscape 320px 高）时 photo 人物会偏小；crowd 满员时手机上名字略挤。下一轮视情况再收。
